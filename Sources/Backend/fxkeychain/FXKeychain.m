...
- (BOOL)setObject:(id)object forKey:(id)key label:(NSString *)label
{
    ...
    if ([_service length]) {
        ...
        query[(__bridge NSString *)kSecClass] = (__bridge id)kSecClassInternetPassword;
        query[(__bridge NSString *)kSecAttrServer] = [self hostname];
        query[(__bridge NSString *)kSecAttrProtocol] = [self protocol];
        query[(__bridge NSString *)kSecAttrPort] = @([self port]);
        query[(__bridge NSString *)kSecAttrAccount] = [key description];
    }
    ...
    if (data)
    {
        query[(__bridge NSString *)kSecAttrLabel] = label;
...
{
    return [self setObject:nil forKey:key label:nil];
}
...
- (id)objectForKey:(id)key
{
    ...
    //generate query
    NSMutableDictionary *query = [NSMutableDictionary dictionary];
    if ([_service length]) query[(__bridge NSString *)kSecAttrService] = [self service];
    if ([_service length]) {
        query[(__bridge NSString *)kSecAttrService] = [self service];
        query[(__bridge NSString *)kSecClass] = (__bridge id)kSecClassGenericPassword;
        query[(__bridge NSString *)kSecAttrAccount] = [key description];
